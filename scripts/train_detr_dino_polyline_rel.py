"""
Train DETR with relative-coordinate polyline head on precomputed DINOv3 embeddings, optionally with diffusion refinement.

Data: Precomputed DINOv3 patch embeddings paired with ground-truth polylines (20 points each) from LiDAR crops.
Dataset: Generated by scripts/data/build_lidar_training_dataset.py.
Differs from train_detr_dino_polyline.py: Predicts per-query bounding box first, then polyline points as offsets relative to box.
Purpose: Adds geometric losses (loss_smooth, loss_len, loss_dir, loss_card) to improve location precision; 3-stage diffusion training.
Note: Still learns distribution but not precise locations; precursor to MapTR variant.
"""
