...
- **The images are about ten years newer than the labels.** Crops come from `Actueel_ortho25`, but 75.3% of the hedge features were drawn from 2014 or 2015 photos (`bronactual`). Unmeasured, and fixable by rebuilding on `2016_ortho25`.
- **LiDAR height does not separate hedges from tree rows.** AHN4 95th-percentile height gives a best single cut of 9.0 m at balanced accuracy 0.638, because Top10NL "heg, haag" includes tall hedgerows, not only clipped hedges.
...
- `exps/probe_lidar_hedge_vs_tree.py`: samples the AHN4 95th-percentile normalized-height raster along features of the `heg` and `bomenrij` layers to test whether height can separate the two classes. Result: medians 8.0 m and 12.2 m, best single cut 9.0 m at balanced accuracy 0.638, so it cannot.
