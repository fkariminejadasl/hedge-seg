...
- **train_detr_unet_polyline.py**: Reimplemented polyline regression without diffusion, using MapTR hierarchical queries (per-polyline instance + per-point embeddings). Initialized backbone with pretrained ResNet18-UNet. Hypothesized hierarchical query design limited precision learning, but results similar to relative-coordinate approach. Note that, **train_detr_maptr_polyline** is similar to train_detr_unet_polyline but with DINOv3 backbone and different dataset and dataloader.
...
- `scripts/train_detr_unet_polyline.py`: successor to `train_detr_maptr_polyline.py`, with the same MapTR-style head (hierarchical queries + iterative reference-point refinement, geometric losses off by default) but a different backbone and data path. The backbone is a ResNet18-UNet loaded from the `train_semseg_unet_resnet18.py` semseg checkpoint (freezable, BatchNorm kept in eval when frozen) running on high-resolution PDOK aerial images, zero-padded 1000 -> 1024; polylines are normalized by the padded extent. Images come from the PDOK dataset (`scripts/data/build_pdok_wms_dataset.py`) and polyline NPZs from `scripts/data/convert_pdok_polylines_to_detr_polyline.py`. Supports flip/rot90 augmentation of image + polylines together (train split only). A single `main(cfg)` supports `cfg.mode` in `{"train", "infer", "preview"}` (preview renders the exact model-input tensors with GT overlaid; inference loads a `cfg.infer_split_file` file list), so there is no separate inference script for this variant.
